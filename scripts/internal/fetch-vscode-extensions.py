#!/usr/bin/env python3
"""
scripts/internal/fetch-vscode-extensions.py

Run on an internet-connected machine to mirror VS Code extensions as .vsix files
for later offline installation via tools/dev-tools/vscode-extensions/setup.sh.

USAGE
-----
  # Mirror whatever is installed in your local VS Code right now:
  python3 scripts/internal/fetch-vscode-extensions.py --from-installed

  # Mirror a specific list of extension IDs:
  python3 scripts/internal/fetch-vscode-extensions.py ms-vscode.cpptools ms-python.python

  # Choose output dir and target platforms:
  python3 scripts/internal/fetch-vscode-extensions.py --from-installed \\
      --out tools/dev-tools/vscode-extensions/vendor \\
      --platforms win32-x64 linux-x64

  # Dry-run (print what would be downloaded, no files written):
  python3 scripts/internal/fetch-vscode-extensions.py --from-installed --dry-run
...
------
  <out>/
    ...
    ...
  manifest-new.json   — drop-in replacement for tools/dev-tools/vscode-extensions/manifest.json
...
def main():
    ...
    ap.add_argument(
        "--out",
        default="tools/dev-tools/vscode-extensions/vendor",
        metavar="DIR",
        help="Output directory for .vsix files (default: tools/dev-tools/vscode-extensions/vendor)",
    )
    ap.add_argument(
        "--platforms",
        nargs="+",
