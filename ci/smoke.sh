...
"$DEVKIT_BINARY" --tools tools --prebuilt prebuilt --no-browser &
SERVER_PID=$!
trap 'kill "$SERVER_PID" 2>/dev/null || true; rm -f devkit.config.json' EXIT

for i in $(seq 1 15); do
  curl -sf http://127.0.0.1:9090/health && echo "Server ready." && break
...
resp=$(curl -sf -H "X-DevKit-Token: $TOKEN" http://127.0.0.1:9090/api/tools)
...
curl -sf -H "X-DevKit-Token: $TOKEN" http://127.0.0.1:9090/api/profiles
